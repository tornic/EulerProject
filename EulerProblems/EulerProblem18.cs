...
namespace EulerProject.EulerProblems
{
    public static class EulerProblem18
    {
        ...
        public static String readableBestPath(List<Coordinate> currentList)
        {
            return String.Join(" => ", currentList.Select((c) => triangleValues[getIndexOfElement(c.row, c.pos)].ToString()).ToList());
        }

        public static int sumOfPath(List<Coordinate> currentList)
        {
            return currentList.Select((c) => triangleValues[getIndexOfElement(c.row, c.pos)]).Sum();
        }
        ...
        public static List<Coordinate> bestPath = new List<Coordinate>();
        public static int bestTotal = 0;
        public static int pathsEvaluated = 0;

        public static void doSomething(List<Coordinate> currentList)
        {
            Coordinate lastCoordinate = currentList[currentList.Count - 1];

            if (currentList[currentList.Count - 1].row == (getNumberOfRows() - 1))
            {
                pathsEvaluated += 1;
                int maxTotal = sumOfPath(currentList);
                if (maxTotal > bestTotal)
                {
                    bestPath = currentList;
                    bestTotal = maxTotal;
                }
            ...
            for (int i = 0; i < getRowMaxPosition(lastCoordinate.row + 1); i++)
            {
                Coordinate currentCoordinate = new Coordinate { pos = i, row = lastCoordinate.row + 1 };

                if (arePositionsAdjacent(lastCoordinate.row, lastCoordinate.pos, currentCoordinate.row, currentCoordinate.pos))
                {
                    List<Coordinate> nextList = new List<Coordinate>(currentList);
                    nextList.Add(currentCoordinate);
                    doSomething(nextList);
                }
            }
        ...
        {
            doSomething(new List<Coordinate>{ new Coordinate { pos=0, row=0 } } );

            String solution = String.Format("{0} = {1} ({2} total paths evaluated)", readableBestPath(bestPath), sumOfPath(bestPath), pathsEvaluated);
